# 12 — APScheduler + Weekly Pipeline + News Monitor

> **Process boundary.** This spec describes the scheduler that runs *inside*
> `plutus-main.service` only. The Telegram bot (`plutus-bot.service`) and the
> Streamlit dashboard (`plutus-dashboard.service`) have **no scheduler**. They
> receive work from `plutus-main` over the loopback push API
> (`http://127.0.0.1:8001/push/...` — see `10_telegram_bot.md`).

---

## Job Inventory — Five APScheduler Jobs (all in `plutus-main`)

| Job ID | Cron (IST) | Function | Purpose |
|---|---|---|---|
| `weekly_pipeline` | `Sun 18:00` | `weekly_pipeline()` | Full research run: universe screen → top-20 agent analysis → write `weekly_runs` + `recommendations` + Markdown report → push summary to bot |
| `weekly_revalidate` | `Mon 09:10` | `weekly_revalidate()` | NEW. Gap re-validation against Monday open. Pure price math, **no LLM**. Downgrades BUY→WATCH/AVOID where appropriate, then pushes a delta message to bot |
| `news_monitor` | `Mon-Fri 09:00–15:59, every 60 min` | `news_monitor()` | Fetch RSS for watchlist + open paper-trade symbols, hard prefilter via `material_keywords.yaml`, batched LLM classify only on survivors, push material alerts to bot |
| `outcome_tracker` | `Mon-Fri 16:30` | `track_recommendation_outcomes()` | Daily outcome update for pending recommendations using IST trading days + stop-first ambiguity rule (see `14_weekly_history.md`) |
| `rejected_headlines_cleanup` | `Daily 03:00` | `cleanup_rejected_headlines()` | NEW. Delete rejected headlines older than `settings.REJECTED_HEADLINES_RETENTION_DAYS` (default 30) |

All cron times are evaluated in `Asia/Kolkata` — the `AsyncIOScheduler` is
constructed with `timezone=IST` and every `CronTrigger` re-asserts
`timezone=IST` for clarity. The host's wall clock can be UTC; the scheduler
does not depend on `TZ`.

---

## `main.py` — Full Implementation

`AsyncIOScheduler` is used (not `BackgroundScheduler`) so APScheduler shares
uvicorn's event loop. This lets `weekly_pipeline()` and friends be `async def`
and call `httpx.AsyncClient` directly.

```python
# src/main.py
"""
Entry point for plutus-main.service.

Runs:
  - FastAPI on settings.API_PORT (default 8000) — public-facing /analyze API
  - AsyncIOScheduler with five jobs (timezone=IST)

Does NOT run:
  - Telegram polling      (plutus-bot.service)
  - Streamlit             (plutus-dashboard.service)
"""
...
def build_scheduler() -> AsyncIOScheduler:
    ...
    # 4. Outcome tracker — Mon-Fri 16:30 IST.
    sched.add_job(
...
- Reads universe from `plutus.data.universe.get_universe()` (Nifty 500 + MidCap
  150 seed CSV with OHLCV-derived liquidity filters — see `_CHANGE_SPEC.md` §2).
- Runs all 5 strategy bundles per symbol via `run_all_bundles(...)`. The
  ranker key is `BundleResult.sharpe_ratio` of the **best** bundle for that
  symbol. Top `settings.BACKTEST_TOP_CANDIDATES` (default 20) advance.
- For each top-20 symbol, `run_analysis(symbol, exchange, use_cache=False)` is
  called — fresh agent run, the symbol-cache layer described in `09_api.md`
  is bypassed.
- DB writes are a single transaction: 1 `weekly_runs` row + N `recommendations`
  rows. `entry_mid` is computed at insert (for outcome tracker).
- Markdown report goes to `src/reports/weekly/<YYYY-MM-DD>.md`. The directory
  is `.gitignore`d (CHANGE_SPEC §0).
- The Telegram summary is **not** composed here. We POST `{run_id}` to the
  bot, and the bot reads back from the DB and formats the message
  (see `10_telegram_bot.md → push_weekly_summary`). On push failure we log a
  warning and continue — the DB write and Markdown report are the source of
  truth; the Telegram message is best-effort.

### `weekly_revalidate()`

- New job, runs Monday 09:10 IST (10 min after NSE open).
- Loads the latest `weekly_runs` row and its BUY+WATCH recommendations.
- For each: `ltp = fetch_live_price(symbol)`.
  - `LTP > entry_high * 1.02` → if BUY, downgrade to **WATCH** with note
    `"gapped past entry +X.X% (LTP ₹...)"`.
  - `LTP < stop_loss` → if BUY, downgrade to **AVOID** with note
    `"broke stop pre-entry (LTP ₹... < SL ₹...)"`.
  - WATCH rows record `revalidated_at` but are not changed.
- Updates `recommendation`, `revalidation_note`, `revalidated_at`.
- **No LLM calls.** `httpx`-based price fetch + arithmetic only.
- Pushes one consolidated delta to plutus-bot
  (`POST /push/revalidation-delta`); the bot formats and sends a single
  Telegram message of the form
  `"📊 Monday open: 1 BUY downgraded (RELIANCE → WATCH, gapped +2.4%)"`.

### `news_monitor()`

- Symbols = `watchlist ∪ open_paper_trades`.
- `classify_news(sym, headlines)` (see `05_data_pipeline.md`) is the integration
  point that:
  - Applies the hard tiered prefilter from `material_keywords.yaml`
    (tiers `MATERIAL_KEYWORD_TIERS`, default `A,B`; stoplist always
    overrides).
  - Persists every dropped headline to the `rejected_headlines` table with
    `filter_status ∈ {'stoplist','no_keyword'}`.
  - Only invokes the batched LLM call when at least one headline survives the
    prefilter — so quiet symbols are zero-LLM.
- On material classification we insert a `news_events` row with
  `is_material=True, alert_sent=False`, push the `event_id` to plutus-bot,
  and only flip `alert_sent=True` after a successful push. If the bot is down
  the row stays `alert_sent=False` and will be retried on next pass for the
  same headline (the dedupe check ignores rows with `alert_sent=False`).

### `track_recommendation_outcomes()`

This function is **defined in `plutus/weekly/outcomes.py`** and re-shown in
`14_weekly_history.md` (the canonical copy). The scheduler in `main.py` simply
imports it and schedules it. Behavioural contract (per CHANGE_SPEC §8):

- Trading-day count uses `Asia/Kolkata`, not UTC, via
  `nse_trading_days_between(start_ist, end_ist)` and
  `data/nse_holidays.txt`.
- Fill price = `rec.entry_mid` (falling back to `entry_high`).
- Same-bar collision (High ≥ target AND Low ≤ stop on one daily candle) is
  resolved as **stop wins** — the conservative outcome.
- Window is `[hold_days_min, hold_days_max]`; before `min` the rec stays
  PENDING, after `max` with no hit it goes EXPIRED at last close.

### `cleanup_rejected_headlines()`

```python
DELETE FROM rejected_headlines
WHERE rejected_at < NOW() - (settings.REJECTED_HEADLINES_RETENTION_DAYS || ' days')::interval
```

Default 30 days, override via `.env`. Job logs the row count it deleted.
...
```bash
# 1. Sunday weekly research run, on demand
python -c "import asyncio; from main import weekly_pipeline; asyncio.run(weekly_pipeline())"

# 2. Monday gap re-validation, on demand
python -c "import asyncio; from main import weekly_revalidate; asyncio.run(weekly_revalidate())"

# 3. Hourly news scan, on demand
python -c "import asyncio; from main import news_monitor; asyncio.run(news_monitor())"

# 4. Outcome tracker, on demand
python -c "import asyncio; from plutus.weekly.outcomes import track_recommendation_outcomes; asyncio.run(track_recommendation_outcomes())"

# 5. rejected_headlines prune, on demand
python -c "import asyncio; from main import cleanup_rejected_headlines; asyncio.run(cleanup_rejected_headlines())"
```
...
- `10_telegram_bot.md` — `/push/weekly-summary`, `/push/news-alert`, and the
  `/push/revalidation-delta` route handlers in plutus-bot.
- `14_weekly_history.md` — canonical `track_recommendation_outcomes()` + the
  `nse_trading_days_between` helper.
- `_CHANGE_SPEC.md` §4 (Monday revalidation), §9 (3 services), §10 (5 jobs).
- `15_deployment.md` — systemd unit for `plutus-main.service` that runs this
  `main.py`.
