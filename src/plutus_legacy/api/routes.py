...
class AnalyzeRequest(BaseModel):
    symbol: str = Field(
        ..., description="NSE stock symbol e.g. RELIANCE, INFY, TATAMOTORS"
    )
...
async def analyze_stock(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Analysis failed: {type(e).__name__}: {str(e)}"
        )
    ...
    return AnalyzeResponse(
        ...
        position=PositionModel(
            **result.get(
                "position",
                {"shares": 0, "capital": 0, "pct_of_portfolio": 0, "max_loss_inr": 0},
            )
        ),
...
async def get_weekly_recommendations(
    ...
    with SessionLocal() as db:
        ...
        watch_signals = [
            ...
            for r in recs
            if r.recommendation.value == "WATCH"
        ]
