...
def _build_suggestion_text(
    dimension: str, value: str, current_wr: float, n: int
) -> str:
    direction = (
        "underperforming" if current_wr < EXPECTED_WIN_RATE else "overperforming"
    )
...
def apply_suggestion(suggestion_id: int, db_session=None) -> bool:
    ...
    try:
        ...
        ctx.commit()
        log.info(
            "Applied suggestion #%d: %s/%s",
            suggestion_id,
            suggestion.dimension,
            suggestion.dimension_value,
        )
        return True
    finally:
        if close_ctx:
