...
def get_sector(symbol: str) -> Optional[str]:
    ...
    if data:
        sector = data.get("sector") or data.get("data", {}).get("sector") or None
        if sector:
...
def get_beta(symbol: str) -> Optional[float]:
    ...
    if data:
        beta_raw = data.get("beta") or data.get("data", {}).get("beta")
...
def _parse_mf_holdings(raw: dict) -> dict:
    ...
    try:
        ...
        if isinstance(holders, list):
            mf_entries = [
                h for h in holders if "mutual" in str(h.get("type", "")).lower()
            ]
        ...
        if changes:
            ...
            verdict = (
                "ACCUMULATING"
                if avg_change > 2.0
                else "REDUCING" if avg_change < -2.0 else "NEUTRAL"
            )
            return {
                "verdict": verdict,
