...
NSE_FNO_BAN_URL = (
    "https://www.nseindia.com/api/liveEquity-derivatives" "?index=fno_ban_list_active"
)
...
def get_universe(use_cache: bool = True, seed_csv: str | None = None) -> List[str]:
    ...
    for symbol in seed:
        ...
        if not (
            settings.UNIVERSE_PRICE_MIN <= last_close <= settings.UNIVERSE_PRICE_MAX
        ):
            continue
