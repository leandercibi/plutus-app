...
async def cmd_signals(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with SessionLocal() as db:
        ...
        if not latest:
            await update.message.reply_text(
                "No weekly analysis yet. Run will trigger Sunday 6PM."
            )
            return
...
async def cmd_stock(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    symbol = args[0].upper()
    await update.message.reply_text(
        f"🔍 Analysing *{symbol}*... ⏳ (~20 sec)", parse_mode=ParseMode.MARKDOWN
    )
...
async def cmd_buy(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 4:
        await update.message.reply_text(
            "Usage: /buy PORTFOLIO SYMBOL PRICE SHARES\nExample: /buy aggressive_momentum RELIANCE 2389 42"
        )
        return

    portfolio_name, symbol, price_str, shares_str = (
        args[0],
        args[1].upper(),
        args[2],
        args[3],
    ...
    with SessionLocal() as db:
        if sub == "add" and len(args) >= 2:
            ...
            if existing:
                ...
                db.commit()
                await update.message.reply_text(
                    f"✅ `{symbol}` added to watchlist.\nYou'll receive news alerts for this stock.",
                    parse_mode=ParseMode.MARKDOWN,
                )
            ...
            if not items:
                await update.message.reply_text(
                    "Watchlist is empty. Add stocks: /watch add RELIANCE"
                )
            else:
...
async def send_weekly_summary(buy_list: list, watch_list: list, market_regime: str):
    ...
    if buy_list:
        msg += (
            f"✅ *BUY ({len(buy_list)}):* "
            + ", ".join(f"`{s}`" for s in buy_list[:6])
            + "\n"
        )
    if watch_list:
        msg += (
            f"⏳ *WATCH ({len(watch_list)}):* "
            + ", ".join(f"`{s}`" for s in watch_list[:4])
            + "\n"
        )
...
async def send_news_alert(
    symbol: str, headline: str, sentiment: str, current_price: float, impact: str
):
    """Push urgent news alert."""
    icon = "🚨" if sentiment == "negative" else "📢"
    signal = (
        "⬇️ SELL/EXIT if holding" if sentiment == "negative" else "📈 Monitor closely"
    )
    msg = (
...
def _format_analysis(symbol: str, result: dict) -> str:
    ...
    icon = {"BUY": "✅", "SELL": "🔴", "HOLD": "⏸", "WATCH": "👀", "AVOID": "❌"}.get(
        rec, "❓"
    )
