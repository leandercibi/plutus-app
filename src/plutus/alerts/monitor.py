...
from plutus.db.models import (
    Alert, AlertType, MockPortfolio, PaperTrade, Recommendation, TradeStatus,
)
...
def _fire_alert(
    ...
    db.add(alert)
    db.commit()
    log.info("Alert fired: %s %s ltp=%.2f channels=%s", trade.symbol, alert_type.value, ltp, sent_names)
...
def check_open_positions(channels=None) -> int:
    ...
    with SessionLocal() as db:
        ...
        for trade in open_trades:
            ...
            # ── TARGET 1 HIT ─────────────────────────────────────────────
            if t1 and ltp >= t1 and not _already_sent(db, trade.id, AlertType.TARGET1_HIT):
            ...
            # ── TARGET 2 HIT ─────────────────────────────────────────────
            if t2 and ltp >= t2 and not _already_sent(db, trade.id, AlertType.TARGET2_HIT):
                gain_pct = (ltp - entry) / entry * 100
                msg = (
            ...
            if days_held >= 5 and not _already_sent(db, trade.id, AlertType.TREND_INVALIDATED):
                ...
                if ema20 and ltp < ema20:
                    ...
                    _fire_alert(db, trade, AlertType.TREND_INVALIDATED, msg, ltp, channels)
                    fired += 1
