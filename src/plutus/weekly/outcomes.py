...
def _evaluate(rec: Recommendation, today: date) -> Optional[dict]:
    ...
    for bar_ts, row in df.iterrows():
        ...
        if low <= stop:
            ...
            verdict = OutcomeVerdict.WRONG_DIRECTION if wrong else OutcomeVerdict.STOPPED
            return {
                "outcome": verdict,
    ...
    try:
        ...
        for rec in pending:
            try:
                ...
                db_session.add(TradeOutcomesAudit(
                ...
                logger.warning("outcome_tracker_rec_failed", rec_id=rec.id, error=str(exc))
                skipped += 1

        db_session.commit()
        logger.info("outcome_tracker_done", total=total, updated=updated, skipped=skipped)
