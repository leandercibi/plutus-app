...
class CalibrationReport:
    ...
    diverging_buckets: List[BucketStats] = field(default_factory=list)  # candidates for suggestion loop
...
def _find_diverging(stats: List[BucketStats], expected_win_rate: float = 0.55) -> List[BucketStats]:
...
def format_report(report: CalibrationReport) -> str:
    ...
    if report.top_best_calls:
        ...
        for r in report.top_best_calls:
            lines.append(f"- {r.symbol} ({r.bundle_used}) → {r.outcome} +{r.outcome_pct:.1f}%")
        lines.append("")
    ...
    if report.diverging_buckets:
        lines.append(f"### ⚠️ Diverging Buckets (>{DIVERGENCE_THRESHOLD:.0f}pp from expected {report.total_closed_trades and 55}%)")
