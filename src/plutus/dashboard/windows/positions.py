...
from plutus.dashboard.api_client import fetch_ltp, log_real_fill, manual_exit
...
def _pnl_html(ltp: float | None, avg_entry: float, qty: int) -> str:
    if ltp is None or avg_entry == 0 or qty == 0:
        return ""
    pnl_inr = (ltp - avg_entry) * qty
    pnl_pct = (ltp - avg_entry) / avg_entry * 100
    color = BUY_GREEN if pnl_inr >= 0 else LOSS_RED
    sign = "+" if pnl_inr >= 0 else ""
    return (
        f'<div style="font-size:13px; color:{color}; font-weight:500;">'
        f'{sign}{_inr(pnl_inr)} ({sign}{pnl_pct:.1f}%)</div>'
        f'<div style="font-size:11px; color:{TEXT_TERTIARY};">'
        f'{qty} shares · avg {_inr(avg_entry)} · LTP {_inr(ltp)}</div>'
    )
...
{_pnl_html(ltp, avg_entry, pos.qty)}""",
  ...
  <div class="plutus-bar-fill" style="width:{int(progress*100)}%; background:{BUY_GREEN};"></div>
        ...
        with b1.popover("📋 Log real fill", use_container_width=True):
            with st.form(key=f"posfill_form_{pos.trade_id}", border=False):
                ...
                default_qty = pos.qty if pos.qty > 0 else 1
                qty = st.number_input("Qty", min_value=1, value=default_qty, key=f"posqty_{pos.trade_id}")
                default_price = round(ltp, 2) if ltp else 100.0
                price = st.number_input(
                    "Price", min_value=0.01, value=default_price, step=0.05, key=f"posprice_{pos.trade_id}"
                )
                total = float(qty) * float(price)
                st.caption(f"Total: {_inr(total)}")
        ...
        with b2.popover("✖ Manual exit", use_container_width=True):
            with st.form(key=f"posexit_form_{pos.trade_id}", border=False):
                ...
                default_qty = pos.qty if pos.qty > 0 else 1
                exit_qty = st.number_input("Qty to sell", min_value=1, value=default_qty, key=f"posexitqty_{pos.trade_id}")
                default_price = round(ltp, 2) if ltp else 100.0
                exit_price = st.number_input(
                    "Exit price", min_value=0.01, value=default_price, step=0.05, key=f"posexitprice_{pos.trade_id}"
                )
                total = float(exit_qty) * float(exit_price)
                st.caption(f"Total proceeds: {_inr(total)}")
                if st.form_submit_button("Close trade", type="primary"):
                    ok, msg = manual_exit(pos.trade_id, reason, qty=int(exit_qty), price=exit_price)
                    (st.success if ok else st.error)(msg)
