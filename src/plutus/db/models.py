...
from sqlalchemy import (
    ...
    Text, ForeignKey, Enum as SAEnum, Date, Index, JSON,
)
...
class WeeklyRun(Base):
    ...
    run_type = Column(String(20), nullable=False, default="scheduled")  # 'scheduled' | 'manual'
    ...
    params_version_id = Column(String(16), nullable=True)  # hash of trading_params at run time
...
class MockPortfolio(Base):
    ...
    def current_cash(self) -> float:
        ...
        open_capital = sum(
            float(t.capital_used or 0) for t in self.trades
            if t.status == TradeStatus.OPEN
        )
        closed_pnl = sum(
            float(t.realised_pnl or 0) for t in self.trades
...
class MarketRegimeSnapshot(Base):
    ...
    distance_from_ema50_pct = Column(Float, nullable=True)        # % distance of Close from EMA50
...
class TradeOutcomesAudit(Base):
    ...
    regime_at_signal = Column(String(20), nullable=True)          # BULL/BEAR/SIDEWAYS at signal time
    created_at = Column(DateTime, default=datetime.utcnow)
...
class WalkForwardRun(Base):
    ...
    overfit_flag = Column(Boolean, default=False)     # True if OOS Sharpe drops >50% from IS
    created_at = Column(DateTime, default=datetime.utcnow)
...
class TradingParam(Base):
    ...
    value_type = Column(String(10), nullable=False, default="float")  # 'float' | 'int' | 'str'
    min_allowed = Column(Float, nullable=True)
    max_allowed = Column(Float, nullable=True)
    label = Column(String(100), nullable=True)     # human-readable label for the Settings form
...
class TuningSuggestion(Base):
    ...
    dimension = Column(String(50), nullable=False, index=True)  # 'score_bucket' | 'bundle' | 'regime'
    dimension_value = Column(String(50), nullable=False)        # e.g. '70-100', 'trend', 'BULL'
    ...
    status = Column(String(20), nullable=False, default="pending")  # pending | applied | rejected | deferred
    applied_at = Column(DateTime, nullable=True)

    __table_args__ = (
        Index("idx_ts_dimension_date", "dimension", "report_date"),
    )
...
class TuningHistory(Base):
    ...
    id = Column(Integer, primary_key=True)
    suggestion_id = Column(Integer, ForeignKey("tuning_suggestions.id"), nullable=False, index=True)
