...
def fetch_bhav_copy(trading_date: date | None = None) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Bhav copy missing expected columns: {missing}. Got: {list(df.columns)}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Refresh seed universe CSV from NSE bhav copy")
    parser.add_argument("--write", action="store_true", help="Overwrite seed_universe_v2.csv")
    parser.add_argument("--angel", action="store_true", help="Enrich with Angel One 52W high (uses up to 50 API calls)")
    parser.add_argument("--date", help="Trading date YYYY-MM-DD (default: last trading day)")
