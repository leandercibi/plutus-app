...
def get_nifty_regime(force: bool = False) -> dict:
    ...
    distance_pct = (close_today - ema50_today) / ema50_today * 100 if ema50_today != 0 else 0.0
...
def get_sector_strength(force: bool = False) -> dict[str, float]:
    ...
    if len(nifty_close) < 31:
        logger.warning("Nifty has only %d bars — sector RS unreliable", len(nifty_close))
...
def persist_regime_snapshot(db_session, snapshot_date: Optional[date] = None):
    ...
    existing = db_session.query(MarketRegimeSnapshot).filter_by(snapshot_date=today).first()
    if existing:
