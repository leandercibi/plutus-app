...
def _fetch_jugaad(symbol: str, days: int) -> pd.DataFrame:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
        future = ex.submit(stock_df, symbol=bare, from_date=start_date, to_date=end_date, series="EQ")
        try:
    ...
    if interval == "1d" and exchange == "NSE":
        ...
        if settings.ANGEL_API_KEY and settings.ANGEL_CLIENT_ID:
            try:
                ...
                logger.warning("Angel One failed for %s: %s — falling back to jugaad", symbol, e)
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return df.dropna(subset=subset) if subset else df.iloc[0:0]  # empty if no indicators computed
