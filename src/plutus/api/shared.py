...
@router.get("/regime", response_model=RegimeVerdictOut)
def get_latest_regime(db: Session = Depends(get_db)) -> RegimeVerdictOut:
    row = db.execute(
    ...
    return RegimeVerdictOut(
        as_of_date=row.as_of_date,
        label=cast(Any, row.label),
        india_vix=row.india_vix,
        breadth_confirmed_flip=row.breadth_confirmed_flip,
    )
...
def _fetch_live_prices(
    ...
    """Fetch LTP via Angel One (1 req/sec). Falls back to yfinance. Stores in latest_price."""
    import logging
