...
from plutus.backtesting.runner import BUNDLE_MAP, _summarise, _empty_result, BundleResult
    ...
    *,
    ...
    if not windows:
        return WalkForwardSummary(
            symbol=symbol, bundle_name=bundle_name, windows=[],
            mean_is_sharpe=0.0, mean_oos_sharpe=0.0,
            overfit_window_count=0, overfit_pct=0.0, verdict="NO_DATA",
        )
...
def _print_summary(summary: WalkForwardSummary) -> None:
    ...
    print(f"{'='*60}")
    print(f"{'Win':>4}  {'IS Start':>10}  {'IS End':>10}  {'OOS Start':>10}  "
          f"{'IS Sh':>7}  {'OOS Sh':>7}  {'IS Tr':>6}  {'OOS Tr':>6}  {'Flag':>6}")
    print("-" * 80)
    for w in summary.windows:
        flag = "OVERFIT" if w.overfit_flag else "ok"
        print(f"{w.window_idx:>4}  {str(w.is_start):>10}  {str(w.is_end):>10}  "
    ...
    print(f"Overfit windows:  {summary.overfit_window_count}/{len(summary.windows)} "
...
def _cli() -> None:
    parser = argparse.ArgumentParser(description="Walk-forward backtest for a bundle/symbol.")
    parser.add_argument("--symbol",  required=True)
    parser.add_argument("--bundle",  required=True, choices=list(BUNDLE_MAP))
    parser.add_argument("--window",  type=int, default=60,  help="IS window in trading days (default 60)")
    parser.add_argument("--step",    type=int, default=7,   help="Roll step in trading days (default 7)")
    parser.add_argument("--oos",     type=int, default=30,  help="OOS window in trading days (default 30)")
    parser.add_argument("--days",    type=int, default=None, help="Total OHLCV days to fetch (auto if omitted)")
