...
from plutus.weekly.outcomes import (
    _evaluate,
    _score_bucket,
...
def _df_from_bars(
    signal_date: date, bars: list[tuple[float, float, float]]
) -> pd.DataFrame:
    """
    ...
    for low, high, close in bars:
        ...
        d += timedelta(days=1)
    idx = pd.date_range(
        start=signal_date + timedelta(days=1), periods=len(bars), freq="D"
    )
    ...
    container = _patch_ohlcv(
        ...
        [
            (
                ...
                121,
                120,
            ),  # day 1 — high touches T2 (checked after SL and before T1 in code)
        ],
    )
    container["rec"] = rec
    result = _evaluate(rec, date.today())
