...
class TestWeeklyRunModel:
    ...
    def test_weekly_run_multiple_runs_same_date_allowed(self, test_db_session):
        ...
        run1 = WeeklyRun(
            run_date=date(2026, 5, 25), run_type="scheduled", stocks_screened=100
        )
        run2 = WeeklyRun(
            run_date=date(2026, 5, 25), run_type="manual", stocks_screened=150
        )
        test_db_session.add_all([run1, run2])
        test_db_session.commit()
        assert run1.id != run2.id

    def test_weekly_run_recommendations_relationship(
        self, weekly_run_factory, recommendation_factory
    ):
...
class TestRecommendationModel:
    ...
    def test_recommendation_outcome_tracking(
        self, recommendation_factory, test_db_session
    ):
...
class TestMockPortfolioModel:
    ...
    def test_portfolio_current_cash_with_open_trades(
        self, mock_portfolio_factory, paper_trade_factory
    ):
    ...
    def test_portfolio_current_cash_with_closed_trades(
        self, mock_portfolio_factory, paper_trade_factory
    ):
    ...
    def test_portfolio_total_realised_pnl(
        self, mock_portfolio_factory, paper_trade_factory
    ):
...
class TestPaperTradeModel:
    ...
    def test_paper_trade_with_recommendation(
        self, mock_portfolio_factory, recommendation_factory, test_db_session
    ):
    ...
    def test_paper_trade_closed_with_profit(
        self, mock_portfolio_factory, test_db_session
    ):
    ...
    def test_paper_trade_closed_with_loss(
        self, mock_portfolio_factory, test_db_session
    ):
