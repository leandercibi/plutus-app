...
def test_classification_buy():
    ...
    assert (
        _classify(b75, position_size=10, material_negative=False, regime_bear=False)
        == Classification.BUY
    )
...
def test_classification_watch():
    ...
    assert (
        _classify(b60, position_size=10, material_negative=False, regime_bear=False)
...
def test_classification_hold():
    ...
    assert (
        _classify(b40, position_size=10, material_negative=False, regime_bear=False)
        == Classification.HOLD
    )
...
def test_classification_avoid_low():
    ...
    assert (
        _classify(b20, position_size=10, material_negative=False, regime_bear=False)
        == Classification.AVOID
    )


def test_material_negative_forces_avoid():
    b_high = ScoreBreakdown(
        95, 90, 0, 90, 95, hard_avoid_reasons=("material_negative_event",)
    )
    result = _classify(
        b_high, position_size=10, material_negative=True, regime_bear=False
    )
...
def test_compute_score_spread_on_fixture(five_symbol_indicators):
    ...
    contexts = {
        "RELIANCE": (
            ...
            "IT",
            {
                ...
                "t1": 1120,
                "t2": 1180,
            },  # R:R=2.0→score≈0 (entry-stop=60, t1-entry=120)
            3.0,
            2.5,
        ),
        "HDFCBANK": (
    ...
    assert (
        spread >= 40
    ), f"Composite spread only {spread} — expected ≥ 40. Scores: {scores}"
...
def test_fixture_not_all_watch(five_symbol_indicators):
    ...
    for sym, df in five_symbol_indicators.items():
        _, cls = compute_score(
            ...
            dii=dii,
            mf=mf,
            nifty_regime=BULL_REGIME,
            sector="IT",
            sector_rs=SECTOR_RS,
            best_bundle_sharpe=1.5,
            position_size=5,
        )
    ...
    assert (
        len(cls_set) > 1
    ), f"All symbols got same classification — all-WATCH collapse: {classifications}"
