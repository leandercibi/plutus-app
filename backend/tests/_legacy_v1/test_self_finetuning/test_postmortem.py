...
def _add_audit_rows(
    ...
    *,
    ...
    recs = (
        ...
        .limit(n_win + n_loss + n_wrong_dir)
        .all()
    )
    recs.reverse()
...
class TestRunPostmortem:
    ...
    def test_correct_win_rate_in_bucket(self, db):
        ...
        report = run_postmortem(lookback_days=30, db_session=db)
        bucket = next(
            (s for s in report.score_bucket_stats if s.value == "70-100"), None
        )
    ...
    def test_mfe_mae_averaged(self, db):
        ...
        report = run_postmortem(lookback_days=30, db_session=db)
        bucket = next(
            (s for s in report.score_bucket_stats if s.value == "55-69"), None
        )
        assert bucket is not None
...
class TestSuggestionLoop:
    ...
    def test_suggestion_has_correct_fields(self, db):
        ...
        suggestion = (
            ...
            .filter(TuningSuggestion.dimension_value == "35-54")
            .first()
        )
    ...
    def test_no_duplicate_suggestion_within_14_days(self, db):
        ...
        count = (
            db.query(TuningSuggestion)
            .filter(TuningSuggestion.dimension_value == "55-69")
            .count()
        )
...
class TestApplySuggestion:
    ...
    def test_apply_writes_history_row(self, db):
        ...
        history = (
            db.query(TuningHistory).filter(TuningHistory.suggestion_id == sid).first()
        )
