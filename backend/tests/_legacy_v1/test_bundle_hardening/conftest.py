...
def make_reversal_df(n: int = 250, seed: int = 13) -> pd.DataFrame:
    ...
    for cycle in range(3):
        ...
        for i in range(20):
            ...
            opens_l.append(o)
            highs_l.append(c * 1.005)
            lows_l.append(o * 0.995)
            vols.append(float(rng.integers(800_000, 1_200_000)))

    total = len(prices)
    return pd.DataFrame(
        {
            "Open": np.array(opens_l[:n]),
            "High": np.array(highs_l[:n]),
            "Low": np.array(lows_l[:n]),
...
def make_volatile_df(n: int = 200, seed: int = 99) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
            "Open": np.array(opens_l),
            "High": np.array(highs_l),
            "Low": np.array(lows_l),
            "Close": np.array(closes_l),
...
def make_pead_df(n: int = 200, seed: int = 55) -> pd.DataFrame:
    ...
    highs.append(gap_close * 1.010)
    lows.append(gap_close * 0.995)
