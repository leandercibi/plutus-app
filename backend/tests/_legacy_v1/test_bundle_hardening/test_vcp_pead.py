...
class TestPEADBundle:
    def test_generates_trades(self, pead_df):
        ...
        ), "PEADBundle: no trades on engineered PEAD data"

    def test_bear_regime_suppresses_signals(self, pead_df):
        strat = _run(
            PEADBundle, pead_df, earnings_months_only=False, nifty_trend="BEAR"
        )
        assert len(strat.trade_log) == 0

    def test_vol_gate_kills_signals(self, pead_df):
        strat = _run(
            PEADBundle, pead_df, earnings_months_only=False, vol_min_ratio=100.0
        )
        assert len(strat.trade_log) == 0
