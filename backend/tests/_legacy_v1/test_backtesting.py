...
class TestSelectBestBundles:
    ...
    def test_select_best_bundles_demotes_zero_trades(self):
        ...
        results = {
            "trend": BundleResult(
                "trend", 0.0, 0.0, 0.0, 3.0, 0, []
            ),  # High Sharpe but 0 trades
            "reversal": BundleResult("reversal", 0.5, 1.8, 8.0, 0.8, 8, []),
            "breakout": BundleResult("breakout", 0.7, 3.2, 12.0, 1.2, 12, []),
            "smc": BundleResult(
                "smc", 0.0, 0.0, 0.0, 2.5, 0, []
            ),  # High Sharpe but 0 trades
...
class TestWeeklyPipeline:
    ...
    def test_weekly_pipeline_returns_top_20(
        self, mock_universe, mock_run_all, mock_save
    ):
        ...
        def mock_results(symbol, days):
            ...
            return {
                ...
                "reversal": BundleResult(
                    "reversal", 0.5, 1.5, 8.0, sharpe - 0.2, 8, []
                ),
                "breakout": BundleResult(
                    "breakout", 0.7, 2.5, 12.0, sharpe - 0.1, 12, []
                ),
                "smc": BundleResult("smc", 0.55, 1.8, 9.0, sharpe - 0.3, 9, []),
                "composite": BundleResult(
                    "composite", 0.65, 2.2, 11.0, sharpe - 0.15, 11, []
                ),
            }
    ...
    @patch("plutus.backtesting.runner.run_all_bundles")
    @patch("plutus.backtesting.runner.get_universe")
    def test_weekly_pipeline_filters_negative_sharpe(
        self, mock_universe, mock_run_all, mock_save
    ):
        ...
        def mock_results(symbol, days):
            if symbol == "SYM1":
                ...
                return {
                    ...
                    "composite": BundleResult(
                        "composite", 0.65, 2.2, 11.0, 1.6, 11, []
                    ),
                }
            else:
...
class TestPaperTrader:
    ...
    def test_paper_trader_buy_success(self, mock_session_local):
        ...
        result = trader.buy(
            symbol="RELIANCE", price=2500.0, shares=10, strategy_used="trend"
        )
