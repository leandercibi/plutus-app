...
def test_separate_rows_per_kind_for_same_symbol(session: Session) -> None:
    session.add(
        AlertCooldown(
            symbol="INFY", kind="SL_WARNING", last_fired_at=datetime(2025, 1, 1, 10)
        )
    )
    session.add(
        AlertCooldown(
            symbol="INFY", kind="SL_BREACH", last_fired_at=datetime(2025, 1, 1, 11)
        )
    )
    session.commit()
...
def test_fetching_warning_does_not_affect_breach(session: Session) -> None:
    ...
    session.add(
        AlertCooldown(
            symbol="TCS", kind="SL_WARNING", last_fired_at=datetime(2025, 1, 1, 10)
        )
    )
    session.add(
        AlertCooldown(
            symbol="TCS", kind="SL_BREACH", last_fired_at=datetime(2025, 1, 1, 9)
        )
    )
    session.commit()
    warning = (
        session.query(AlertCooldown).filter_by(symbol="TCS", kind="SL_WARNING").one()
    )
    breach = (
        session.query(AlertCooldown).filter_by(symbol="TCS", kind="SL_BREACH").one()
    )
...
def test_invalid_kind_rejected(session: Session) -> None:
    session.add(
        AlertCooldown(symbol="X", kind="BOGUS", last_fired_at=datetime(2025, 1, 1, 10))
    )
