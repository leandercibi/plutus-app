...
def test_filter_by_label_buy(client: TestClient, auth_headers: dict[str, str]) -> None:
...
def test_filter_by_run_id(client: TestClient, auth_headers: dict[str, str]) -> None:
    resp = client.get("/swing/signals?run_id=run-1", headers=auth_headers)
    assert resp.status_code == 200
    assert len(resp.json()) == 2


def test_get_single_signal(client: TestClient, auth_headers: dict[str, str]) -> None:
    resp = client.get("/swing/signals/1", headers=auth_headers)
    assert resp.status_code == 200
    assert resp.json()["id"] == 1


def test_get_missing_signal_404(client: TestClient, auth_headers: dict[str, str]) -> None:
