...
def test_manual_exit_updates_state(
...
def test_manual_exit_does_not_error_or_refire(
    ...
    first = client.post(
        "/swing/trades/1/exit/manual", json={"reason": "x"}, headers=auth_headers
    )
    second = client.post(
        "/swing/trades/1/exit/manual", json={"reason": "y"}, headers=auth_headers
    )
    assert first.status_code == 200
    assert second.status_code == 200


def test_manual_exit_unknown_trade_404(
    client: TestClient, auth_headers: dict[str, str]
) -> None:
    resp = client.post(
        "/swing/trades/9999/exit/manual", json={"reason": "x"}, headers=auth_headers
    )
