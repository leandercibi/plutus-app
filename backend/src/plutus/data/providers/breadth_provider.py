...
_SYMBOLS = [
    ...
    "TITAN.NS", "HCLTECH.NS", "SUNPHARMA.NS", "WIPRO.NS", "ULTRACEMCO.NS",
    "NESTLEIND.NS", "TECHM.NS", "BAJAJFINSV.NS", "NTPC.NS", "POWERGRID.NS",
    "ONGC.NS", "COALINDIA.NS", "JSWSTEEL.NS", "HEROMOTOCO.NS", "ADANIENT.NS",
]
...
def _load_cache(key: str) -> pd.DataFrame | None:
    ...
    if not p.exists():
        return None
    age_hours = (pd.Timestamp.now() - pd.Timestamp(p.stat().st_mtime, unit="s")).total_seconds() / 3600
...
def _fetch_closes(start: date, end: date) -> pd.DataFrame:
    ...
    try:
        raw = yf.download(
            _SYMBOLS, start=fetch_start, end=end + timedelta(days=1),
