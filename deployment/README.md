...
> **Service split.** Plutus runs as **three** application services plus PostgreSQL on a single OCI A1.flex VM.
...
The deployed tree is rooted at `/home/ubuntu/plutus-app/`. All Python lives under `/home/ubuntu/plutus-app/src/`.
...
Plutus relies on the **APScheduler `timezone=IST` argument exclusively** — every `CronTrigger` in `main.py` is constructed with `timezone=IST` and the scheduler itself is built with `timezone=IST`. The host's wall clock is left in UTC, the OCI default. We deliberately **do not** set `Environment=TZ=Asia/Kolkata` in the unit files because mixing process-local TZ with explicit `pytz` triggers is the easiest way to get phantom double-shifts. One source of truth, in code.

(`pg_dump` filenames will be in UTC — that's intended; consistent across backups regardless of host.)

### Install Service Files

The systemd service files are provided in this deployment directory. Copy them to the system:

```bash
sudo cp /home/ubuntu/plutus-app/deployment/plutus-main.service /etc/systemd/system/
sudo cp /home/ubuntu/plutus-app/deployment/plutus-bot.service /etc/systemd/system/
...
`plutus-bot.service` is ordered `After=plutus-main.service` so the bot's internal HTTP client never tries to reach a not-yet-bound port on first boot. The bot itself only binds `127.0.0.1:8001` for `/push/*` — there is no public ingress for that port.
...
Cloudflare Tunnel gives the dashboard a public URL without exposing the OCI IP and without needing a domain.
...
# Update the credentials-file path in config.yml with your actual tunnel UUID
...
(Restart `plutus-main` first — `plutus-bot` is ordered after it. Restarting all three at once is fine; systemd will resolve the order.)
...
- DO NOT install `ta-lib` (C extension, ARM64 compilation is painful) — use `pandas-ta` instead.
