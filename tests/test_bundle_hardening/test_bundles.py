...
from tests.test_bundle_hardening.conftest import (
    run_strategy, make_bull_df, make_bear_df, make_sideways_df, make_volatile_df
)
...
class TestTrendBundle:
    def test_generates_trades_in_bull(self, bull_df):
        ...
        assert len(strat.trade_log) >= 1, "TrendBundle: no trades on engineered bull data"

    def test_bear_regime_gate_suppresses_signals(self, bull_df):
        strat = _run(TrendBundle, bull_df, nifty_trend="BEAR")
        assert len(strat.trade_log) == 0, "TrendBundle: should produce 0 trades in BEAR regime"
...
class TestReversalBundle:
    def test_generates_trades_on_oscillating(self, sideways_df):
        ...
        assert len(strat.trade_log) >= 1, "ReversalBundle: no trades on oscillating data"
...
class TestBreakoutBundle:
    def test_generates_trades_in_bull(self, bull_df):
        ...
        assert len(strat.trade_log) >= 1, "BreakoutBundle: no trades on engineered bull data"
...
class TestSMCBundle:
    def test_generates_trades_on_volatile(self, volatile_df):
        ...
        assert len(strat.trade_log) >= 1, "SMCBundle: no trades on volatile/liquidity-grab data"
...
@pytest.mark.parametrize("label,cls,df_fn", [
