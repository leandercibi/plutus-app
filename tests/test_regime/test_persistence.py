...
def test_snapshot_unique_per_date(db_session):
    ...
    rows = db_session.query(MarketRegimeSnapshot).filter_by(snapshot_date=date(2026, 6, 2)).all()
    assert len(rows) == 1
...
def test_snapshot_null_sector_rs_allowed(db_session):
    ...
    db_session.add(snap)
    db_session.commit()
    row = db_session.query(MarketRegimeSnapshot).filter_by(snapshot_date=date(2026, 6, 3)).one()
