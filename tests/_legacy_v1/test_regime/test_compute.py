...
from plutus.data.regime import (
    get_nifty_regime,
    get_sector_strength,
    INDEX_YF_MAP,
    SECTOR_DISPLAY,
...
def test_fetch_nifty_50_returns_attrs(monkeypatch, synthetic_bull_nifty_df):
    monkeypatch.setattr(
        regime_mod, "fetch_index_ohlcv", lambda s, **kw: synthetic_bull_nifty_df
    )
...
def test_bull_regime(synthetic_bull_nifty_df, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        regime_mod, "fetch_index_ohlcv", lambda *a, **kw: synthetic_bull_nifty_df
    )
...
def test_bear_regime(synthetic_bear_nifty_df, monkeypatch, tmp_path):
    monkeypatch.setattr(regime_mod, "_REGIME_CACHE_FILE", tmp_path / "regime.json")
    monkeypatch.setattr(
        regime_mod, "fetch_index_ohlcv", lambda *a, **kw: synthetic_bear_nifty_df
    )
    r = get_nifty_regime(force=True)
    assert r["trend"] == "BEAR"
    assert r["slope"] < 0
...
def test_sideways_regime(synthetic_flat_nifty_df, monkeypatch, tmp_path):
    monkeypatch.setattr(regime_mod, "_REGIME_CACHE_FILE", tmp_path / "regime.json")
    monkeypatch.setattr(
        regime_mod, "fetch_index_ohlcv", lambda *a, **kw: synthetic_flat_nifty_df
    )
    r = get_nifty_regime(force=True)
