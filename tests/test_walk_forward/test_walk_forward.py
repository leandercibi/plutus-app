...
from tests.test_bundle_hardening.conftest import make_bull_df, make_sideways_df, make_volatile_df
...
class TestRunWalkForward:
    def test_basic_window_slicing(self):
        ...
        summary = run_walk_forward(
            "TESTSTOCK", "trend",
            window_bars=80, step_bars=20, oos_bars=70,
            df=df,
        )
    ...
    def test_too_small_df_returns_no_data(self):
        ...
        summary = run_walk_forward(
            "TESTSTOCK", "trend",
            window_bars=60, step_bars=7, oos_bars=30,
            df=df,
        )
...
class TestSummaryAggregation:
    def test_mean_sharpe_computed(self):
        ...
        if summary.windows:
            expected_is = sum(w.is_sharpe for w in summary.windows) / len(summary.windows)
...
class TestPersistence:
    def test_persist_creates_rows(self, tmp_path, monkeypatch):
        ...
        summary = run_walk_forward(
            "RELIANCE", "trend",
            window_bars=80, step_bars=20, oos_bars=40,
            df=df,
        )
