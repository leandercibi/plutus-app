...
class TestLoadKeywords:
    def test_loads_and_caches_keywords(self, sample_keywords_yaml, monkeypatch):
        monkeypatch.setattr("plutus.data.news.settings.MATERIAL_KEYWORDS_YAML", str(sample_keywords_yaml))
...
class TestFetchNews:
    ...
    def test_fetches_from_rss_feeds(self, mock_parse, monkeypatch):
        ...
        mock_entry.get = lambda k, default="": {"title": "RELIANCE stock update", "published": "2026-05-30T10:00:00Z", "link": "https://example.com/rss"}.get(k, default)
...
class TestSaveRejectedHeadlines:
    ...
    def test_saves_to_database(self, mock_session_local):
        ...
        rejected = [
            {"headline": "Rejected 1", "source": "test", "published_at": "2026-05-30", "filter_status": "stoplist"},
            {"headline": "Rejected 2", "source": "test", "published_at": "2026-05-30", "filter_status": "no_keyword"},
        ]
...
class TestClassifyNews:
    ...
    def test_calls_llm_for_kept_headlines(self, mock_llm, mock_save, sample_keywords_yaml, monkeypatch):
        monkeypatch.setattr("plutus.data.news.settings.MATERIAL_KEYWORDS_YAML", str(sample_keywords_yaml))
        monkeypatch.setattr("plutus.data.news.settings.MATERIAL_KEYWORD_TIERS", "A")
    ...
    @patch('plutus.data.news.save_rejected_headlines')
    def test_skips_llm_when_all_rejected(self, mock_save, sample_keywords_yaml, monkeypatch):
        monkeypatch.setattr("plutus.data.news.settings.MATERIAL_KEYWORDS_YAML", str(sample_keywords_yaml))
        monkeypatch.setattr("plutus.data.news.settings.MATERIAL_KEYWORD_TIERS", "A")
...
class TestLlmBatchClassify:
    ...
    def test_successful_classification(self, mock_llm, monkeypatch):
        monkeypatch.setattr("plutus.data.news.settings.DEEPSEEK_FAST_MODEL", "test-model")
