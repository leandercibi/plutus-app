...
from plutus.db.models import (
    WeeklyRun, Recommendation, RecommendationVerdict,
    NewsEvent, PaperTrade, Watchlist, TradeStatus,
    RejectedHeadline
)
...
async def test_weekly_pipeline_success(
    ...
    assert len(recs) > 0
    assert all(r.recommendation in (RecommendationVerdict.BUY, RecommendationVerdict.WATCH) for r in recs)
