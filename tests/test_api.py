...
def test_analyze_success(mock_run_analysis, client, valid_api_key, mock_analysis_result):
...
@patch("plutus.api.routes.run_analysis")
def test_analyze_symbol_normalization(mock_run_analysis, client, valid_api_key, mock_analysis_result):
...
@patch("plutus.api.routes.run_analysis")
def test_analyze_default_exchange(mock_run_analysis, client, valid_api_key, mock_analysis_result):
...
def test_weekly_success(mock_session_local, client, valid_api_key, test_db_session, weekly_run_factory, recommendation_factory):
...
@patch("plutus.api.routes.SessionLocal")
def test_weekly_no_run_found(mock_session_local, client, valid_api_key, test_db_session):
...
@patch("plutus.api.routes.SessionLocal")
def test_weekly_empty_recommendations(mock_session_local, client, valid_api_key, test_db_session, weekly_run_factory):
...
@patch("plutus.api.routes.SessionLocal")
def test_weekly_filters_only_buy_watch(mock_session_local, client, valid_api_key, test_db_session, weekly_run_factory, recommendation_factory):
    """Test /weekly only returns BUY and WATCH recommendations."""
    run = weekly_run_factory()
    
    recommendation_factory(symbol="BUY1", recommendation=RecommendationVerdict.BUY, weekly_run_id=run.id)
    recommendation_factory(symbol="WATCH1", recommendation=RecommendationVerdict.WATCH, weekly_run_id=run.id)
    recommendation_factory(symbol="HOLD1", recommendation=RecommendationVerdict.HOLD, weekly_run_id=run.id)
    recommendation_factory(symbol="AVOID1", recommendation=RecommendationVerdict.AVOID, weekly_run_id=run.id)
...
@patch("plutus.api.routes.run_analysis")
def test_analyze_response_model_validation(mock_run_analysis, client, valid_api_key, mock_analysis_result):
    ...
    # Required fields
    required_fields = [
        "symbol", "exchange", "current_price", "recommendation",
        "confidence", "entry_zone", "targets", "stop_loss",
        "risk_reward", "position", "hold_days", "strategy",
        "signals", "risk_flags", "reasoning", "analysis_time_sec"
    ]
