...
class TestAnalyzeCard:
    ...
    def test_renders_all_recommendation_types(self, rec):
        ...
        at = AppTest.from_function(_app_render_card, default_timeout=15, kwargs={"data": data})
        at.run()
        assert not at.exception, f"render_analyze_card raised for rec={rec}: {at.exception}"
    ...
    def test_reasoning_expander_present(self):
        ...
        assert any("Thesis" in lbl for lbl in labels), f"No Thesis expander, got: {labels}"
...
class TestRenderAnalyzeResult:
    ...
    def test_successful_response_renders_card(self):
        from streamlit.testing.v1 import AppTest
        data = _make_analyze_response("BUY")
        at = AppTest.from_function(_app_render_analyze_result_200, default_timeout=15,
                                   kwargs={"data": data})
        at.run()
        assert not at.exception
...
class TestPortfolioHelpers:
    ...
    def test_get_trade_history_maps_closed_trades_to_dicts(self):
        ...
        from plutus.db.models import (
            PaperTrade, TradeStatus, TradeDirection, TradeExitReason
        )
        ...
        db.query.return_value.filter.return_value.all.return_value = []
        db.query.return_value.filter.return_value.order_by.return_value.first.return_value = port
        db.query.return_value.filter.return_value.order_by.return_value.all.return_value = [trade]
        ...
        assert len(result) == 1
        row = result[0]
        expected_keys = {"symbol", "side", "entry_price", "entry_date",
                         "exit_price", "exit_date", "shares",
                         "realised_pnl", "realised_pnl_pct", "exit_reason"}
...
class TestDashboardSmoke:
    ...
    def test_dashboard_loads_no_portfolios(self):
        ...
        with patch("plutus.db.session.SessionLocal", return_value=db), \
             patch("plutus.backtesting.paper_trader.fetch_live_price", return_value=100.0):
    ...
    def test_dashboard_loads_with_portfolio(self):
        ...
        with patch("plutus.db.session.SessionLocal", return_value=db), \
             ...
             patch("plutus.backtesting.paper_trader.fetch_live_price", return_value=2800.0):
            at = AppTest.from_file(_app_path(), default_timeout=30)
            at.run()
