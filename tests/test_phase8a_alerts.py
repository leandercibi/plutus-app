...
class TestTelegramChannel:
    def test_send_success(self):
        ...
        with patch("plutus.alerts.channels.asyncio.run") as mock_run, \
             patch("telegram.Bot"):
            ch = TelegramChannel()
    ...
    def test_send_failure_returns_false(self):
        ...
        with patch("plutus.alerts.channels.asyncio.run", side_effect=Exception("connection refused")), \
             patch("telegram.Bot"):
            ch = TelegramChannel()
            result = ch.send("hello")
...
class TestCheckOpenPositions:
    def _run_with_mock(self, ltp, entry=2800.0, sl=2720.0, t1=2950.0, t2=3100.0,
                       days_held=0, no_rec=False, already_sent_type=None):
    ...
    def test_ltp_fetch_failure_skips_trade_gracefully(self):
        ...
        with patch("plutus.alerts.monitor.SessionLocal", side_effect=db_factory), \
             patch("plutus.alerts.monitor.fetch_live_price", side_effect=Exception("network error")):
            fired = check_open_positions(channels=[])
...
class TestRunMonitor:
    def test_skips_outside_nse_hours(self):
        ...
        with patch("plutus.alerts.monitor._is_nse_hours", return_value=False), \
    ...
    def test_runs_during_nse_hours(self):
        ...
        with patch("plutus.alerts.monitor._is_nse_hours", return_value=True), \
             patch("plutus.alerts.monitor.check_open_positions", return_value=0) as mock_check:
            run_monitor()
