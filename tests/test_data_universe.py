...
class TestLoadSeedSymbols:
    def test_load_valid_csv(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("plutus.data.universe.settings.UNIVERSE_SEED_CSV", str(csv_path))
    ...
    def test_missing_file(self, tmp_path, monkeypatch):
        monkeypatch.setattr("plutus.data.universe.settings.UNIVERSE_SEED_CSV", str(tmp_path / "missing.csv"))
...
class TestLoadFnoBanList:
    ...
    def test_use_stale_file_on_api_failure(self, tmp_path, monkeypatch):
        ...
        with patch('plutus.data.universe.requests.get', side_effect=Exception("API down")):
            banned = universe._load_fno_ban_list()
            assert banned == {"STALE1", "STALE2"}

    def test_empty_when_no_file(self, tmp_path, monkeypatch):
        monkeypatch.setattr("plutus.data.universe.FNO_BAN_FILE", tmp_path / "missing.txt")
...
class TestGetUniverse:
    ...
    def test_filters_banned_symbols(self, mock_save, mock_fetch, mock_ban, mock_seed, mock_cache, monkeypatch):
        ...
        monkeypatch.setattr("plutus.data.universe.settings.UNIVERSE_MIN_AVG_VOLUME", 500000)
        monkeypatch.setattr("plutus.data.universe.settings.UNIVERSE_MIN_AVG_VALUE_CR", 10.0)
    ...
    @patch('plutus.data.universe.fetch_ohlcv')
    @patch('plutus.data.universe._save_cached_universe')
    def test_filters_by_price(self, mock_save, mock_fetch, mock_ban, mock_seed, mock_cache, monkeypatch):
        mock_cache.return_value = None
    ...
    @patch('plutus.data.universe.fetch_ohlcv')
    @patch('plutus.data.universe._save_cached_universe')
    def test_filters_by_volume(self, mock_save, mock_fetch, mock_ban, mock_seed, mock_cache, monkeypatch):
        mock_cache.return_value = None
