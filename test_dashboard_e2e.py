...
def test_dashboard():
    ...
    with sync_playwright() as p:
        ...
        # Check page title
        assert (
            "Plutus" in page.title()
        ), f"Expected 'Plutus' in title, got: {page.title()}"
        ...
        for i, (tab_name, expected_content) in enumerate(tabs):
            ...
            try:
                ...
                if missing:
                    print(
                        f"   ⚠️  Missing: {', '.join(missing[:2])} (expected - DB empty)"
                    )
        ...
        # Take screenshot
        screenshot_path = (
            "/Users/leander/personal-projects/plutus-app/dashboard_test.png"
        )
